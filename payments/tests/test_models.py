...
class PaymentModelTests(TestCase):
    ...
    def test_unique_pending_per_borrowing_and_type_constraint(self):
        ...
        self.create_payment(borrowing_id=1, payment_type=PaymentType.PAYMENT, status=PaymentStatus.PENDING)
        ...
        p_paid = self.create_payment(borrowing_id=1, payment_type=PaymentType.PAYMENT, status=PaymentStatus.PAID)
        self.assertEqual(p_paid.status, PaymentStatus.PAID)

        # And a PENDING with a different type is allowed
        p_other_type = self.create_payment(borrowing_id=1, payment_type=PaymentType.FINE, status=PaymentStatus.PENDING)
